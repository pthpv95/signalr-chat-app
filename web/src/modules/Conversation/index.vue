        ...
        <div class="card">
          <div class="card-header msg_head">
            <div class="d-flex bd-highlight" v-if="selectedContact.id !== ''">
              <i
                class="fas fa-arrow-left back_botton"
                v-if="isMobileScreen"
                @click="onClickBack"
              ></i>
              <div class="user_info">
          ...
          <p v-show="isTyping" class="typing-message">
            {{ this.selectedContact.name }} is typing ...
          </p>
          ...
          <form v-on:submit.prevent="onSubmit" v-if="selectedContact.id !== ''">
            <div class="card-footer">
              <div v-if="selectedFile">
                <img
                  :src="selectedFile.previewSource"
                  width="120px"
                  height="120px"
                />
...
import { getFullNameAlias } from './helper'
...
export default {
  ...
  data() {
    return {
      ...
      isTyping: false,
      typingIcon,
      isMobileScreen: /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
        navigator.userAgent
      ),
  ...
  async mounted() {
    ...
    this.contacts = res.data.map((c) => ({
      ...
      aka: getFullNameAlias(c.firstName, c.lastName),
    }))
  ...
  methods: {
    ...
    handleSearchContact: function(e) {
      ...
      this.searchedContacts = this.contacts.filter(
        (c) =>
